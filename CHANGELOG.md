...
### Added
- **Opt-in oam sandbox: `ELECTRON_MCP_SANDBOX=1`.** The launcher spawns oam under `--permission` with no grants at all: filesystem, child-process and network access are refused at runtime with `ERR_ACCESS_DENIED`, and the environment reads as empty. This server needs none of them, so the sandbox costs no capability and turns anything the server merely happens not to use into a refusal. Two tests keep that true rather than asserted: `src/bundle-surface.test.ts` pins the shipped bundle's built-in imports to exactly `node:process` and `node:module` and its executable code to zero `process.env` reads (the environment leg has to be static, since a hidden variable does not throw), and `src/sandbox.test.ts`, wherever an oam at the floor is installed, calls every tool under bare `--permission` and requires the results to match a plain Node run tool for tool. Off by default, because `--permission` is a behaviour change. A host already running on oam spawns a fresh one, since only a fresh oam can apply a process-level flag; that adds one runtime boot there. Never dropped silently: when the sandbox was requested but is not applied -- no usable oam, the spawn failed, or `ELECTRON_MCP_RUNTIME=node` -- the launcher prints a line saying so, how to get it applied, and that `ELECTRON_MCP_RUNTIME=oam` makes its absence a startup error instead. `true` / `yes` / `on` enable it as well as `1`, `0` / `false` / `no` / `off` disable it, and any other value is treated as off and named on stderr. README gains a "Runtime and sandbox" section with the fail-closed config and the launcher's environment variables. (#25)
