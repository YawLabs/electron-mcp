...
- **The launcher header now documents that no `--permission` sandbox is wired up**, rather than leaving the omission to look like an oversight. The rationale it records — spawning arbitrary app binaries, reading project directories, talking to whatever host:port a DevTools endpoint lands on — is text shared with the other launchers in the fleet and describes more than this server does: its tools take code and config as string input and return generated code or findings, with no process spawn, project-directory read, or network client anywhere in `src/`. No behavior change; the oam spawn argv is untouched.
