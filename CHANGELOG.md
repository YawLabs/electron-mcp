...
### Changed
- Maintenance release; no changes since v1.6.0.

## [1.6.0] — 2026-09-14

### Added
- **Opt-in oam sandbox: `ELECTRON_MCP_SANDBOX=1`.** The launcher spawns oam under `--permission` with no grants at all: filesystem, child-process and network access are refused at runtime with `ERR_ACCESS_DENIED`, and the environment reads as empty. This server needs none of them, so the sandbox costs no capability and turns anything the server merely happens not to use into a refusal. Two tests keep that true rather than asserted: `src/bundle-surface.test.ts` pins the shipped bundle's built-in imports to exactly `node:process` and `node:module` and its executable code to zero `process.env` reads (the environment leg has to be static, since a hidden variable does not throw), and `src/sandbox.test.ts`, wherever an oam at the floor is reachable (`OAM_BIN`, the installed locations, or `PATH`), calls every tool under bare `--permission` and requires the results to match a plain Node run tool for tool -- and `release.sh` refuses to release when that differential was skipped. Off by default, because `--permission` is a behaviour change. A host already running on oam spawns a fresh one, since only a fresh oam can apply a process-level flag; the host's own binary is a candidate, so a bundled oam that is on neither `PATH` nor an installed location (Yaw MCP's) can still sandbox. That adds one runtime boot there, plus a `--version` probe per oam binary found. Never dropped silently: when the sandbox was requested but is not applied, the launcher prints a line saying so and how to get it applied -- under `ELECTRON_MCP_RUNTIME=node`, to remove that setting; otherwise (no usable oam, or the spawn failed), that `ELECTRON_MCP_RUNTIME=oam` makes its absence a startup error instead -- printed only once the launcher is committed to serving, so it never sits above an exit that served nothing. `true` / `yes` / `on` enable it as well as `1`, `0` / `false` / `no` / `off` disable it; that value and `ELECTRON_MCP_RUNTIME` are both trimmed and case-insensitive, and an unrecognised value of either is treated as the default and named on stderr, so a typo cannot quietly turn the fail-closed pairing (`ELECTRON_MCP_SANDBOX=1` + `ELECTRON_MCP_RUNTIME=oam`) into a fail-open one. README gains a "Runtime and sandbox" section with the fail-closed config, how to confirm the sandbox took, and the launcher's environment variables. (#25)

### Changed
- Under `ELECTRON_MCP_RUNTIME=node` on an oam host with no Node on `PATH`, the error now names its cause (`ELECTRON_MCP_RUNTIME=node on oam <version>`; it used to start with an empty subject) and says to put Node on `PATH` instead of suggesting `oam self-update`, which would not help.
- The `ELECTRON_MCP_RUNTIME=oam` "no usable oam" error, when the sandbox is also requested, says the sandbox needs one and qualifies the `ELECTRON_MCP_RUNTIME=node` suggestion (Node cannot apply it), so the advice no longer loops back to the setting the fallback note just recommended.
- Under `ELECTRON_MCP_RUNTIME=node` on an oam host at the floor with no Node on `PATH`, the error now also offers the zero-install fix: remove that setting and serve on the oam already running.
...
- `electron_audit_security` strips HTML comments from `htmlContent` the way a browser reads them: a comment closes at the first `-->` or `--!>`, `<!-->` and `<!--->` are complete empty comments, and an unterminated comment runs to the end of the input. The previous `replace(/<!--[\s\S]*?-->/g)` did not know the alternative closer or the unterminated case, so a CSP meta tag or `<webview>` after one of those was audited as if rendered. Also the shape CodeQL flagged as `js/incomplete-multi-character-sanitization`.
- `scripts/update-manifests.mjs` escapes every value it writes into a Ruby double-quoted string in the Homebrew formula -- backslash, `"`, and the `#{` / `#$` / `#@` interpolation openers -- instead of the quote alone. A package description containing `#{...}` was an interpolation in the generated formula, not text. (CodeQL `js/incomplete-sanitization`.)
...
- **An oam host older than the floor no longer serves the server itself.** When a client ran `oam run bin/electron-mcp.mjs` with an old oam and discovery found nothing usable, or the oam it found would not start, the server ran on that old oam. When a newer oam WAS found, the handoff inherited stdio, which an oam older than 0.9.0 does not honor, so the MCP handshake never answered. An old host now hands off with piped stdio to the newest usable oam, or to Node on `PATH`, or exits with an error when there is neither.
...
- **An oam that fails to launch under `auto` is named on stderr.** When the chosen oam passed its version check but could not be spawned (deleted or replaced in between), the launcher fell back without a word; it now prints `failed to launch oam at <path> (<error>); using Node instead.` first. Stdio piping, signal forwarding and mirroring the child's exit all start only once the child has spawned, so a launch that fails still falls back.
...
- **The launcher pins an oam floor of 0.9.0**, probing `oam --version` before it hands off. Below that floor oam's `child_process` diverged from Node's: `execFile` ran its arguments through a **shell**, so an argument carrying shell metacharacters was re-split and executed; `exec` accepted a `timeout` and ignored it; `spawnSync` truncated at `maxBuffer` while reporting success; and `stdio: 'inherit'`/`'ignore'` both behaved as `'pipe'`. The floor is the same `[0, 9, 0]` every launcher in the fleet carries; this server's own `src/` issues no `child_process` calls, so what it buys here is a guarantee about the runtime underneath rather than a bug this server was hitting. Under the default `ELECTRON_MCP_RUNTIME=auto` an old oam is *not* an error — the launcher falls back to Node and **says so on stderr**, because a silent downgrade is how someone keeps running an oam they meant to update (stderr is safe; MCP frames travel on stdout). `ELECTRON_MCP_RUNTIME=oam` makes it a hard exit instead, naming both remedies (`oam self-update`, or `ELECTRON_MCP_RUNTIME=node`). A *missing* oam under `auto` still falls back with no message, unchanged from 1.3.0.
- **The version probe is the launcher's first subprocess, and it is not on the miss path.** Discovery stays stat-only, so a machine with no oam still costs nothing; the probe runs only once the launcher has already decided to spawn oam anyway — the header records 26ms median (n=12, windows-arm64), paid once per MCP session. Parsing takes the first `major.minor.patch` in the output, so a pre-release (`0.9.0-rc.1`) reads as its base version. A probe that cannot be read at all — not executable, wrong architecture, deleted between the stat and the exec — is treated as below the floor and degrades down the same path, sharing the outdated-oam wording (1.4.1 separates the two).
- **The launcher header now documents that no `--permission` sandbox is wired up**, rather than leaving the omission to look like an oversight. The rationale it records — spawning arbitrary app binaries, reading project directories, talking to whatever host:port a DevTools endpoint lands on — describes more than this server does: its tools take code and config as string input and return generated code or findings, with no process spawn, project-directory read, or network client anywhere in `src/`. No behavior change; the oam spawn argv is untouched.

### Infrastructure

- **`server.json` and `package.json` can no longer drift apart without failing the release.** `server.json` is what the Official MCP Registry reads at publish time, and it carries the version twice — top-level and `packages[].version`. `release.sh` syncs it with `jq` separately from the `npm version` bump, so an edit that updates one and not the other ships a desynced registry entry: visible to users, invisible to the release. Three assertions now live in `src/release-metadata.test.ts` — top-level version parity, per-package version parity (the jq sync writes only `packages[0]`; the assertion covers every entry and first requires the array to be non-empty), and `mcpName` == `server.json` `name`. That last one is a different drift mode: the registry keys the package by `name` while npm consumers read `mcpName`, so disagreement puts discovery and install on different identifiers. It asserts both fields are non-empty *first*, so it cannot pass vacuously if a refactor drops both. Ported from tailscale-mcp, where the check originated in May, as part of a same-day rollout across the `@yawlabs` servers. Its one live firing so far was tailscale-mcp's 0.15.0 run, which aborted at the test gate because `server.json` had been bumped ahead of `package.json` — `release.sh` bumps at step 3 and syncs `server.json` from it, so pre-syncing makes the two disagree at test time in step 2. The suite runs twice per release: `release.sh` runs `npm test` before the bump, and `prepublishOnly` re-runs it on `npm publish`, so the gate also fires *after* the bump, against the files actually being published. Test paths resolve from `import.meta.url` rather than `process.cwd()` — load-bearing, since `npm test` is `cd dist && node --test` — and `tsc` compiles the file into `dist/`, so discovery picks it up with no manifest edit.
...
- `scripts/build-binary.mjs` pins the CLI source entry instead of deriving it from `bin`'s value, which would have resolved to `bin/electron-mcp.ts` once `bin` moved to the launcher — the breakage postgres-mcp shipped in its 0.9.0.
