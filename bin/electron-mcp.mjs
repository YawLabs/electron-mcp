...
if (mode === "node") {
  ...
  if (!oam) {
    if (mode === "oam") {
      ...
      writeSync(
        2,
        "electron-mcp: ELECTRON_MCP_RUNTIME=oam but no oam binary was found.\n" +
    ...
    if (mode === "oam") {
      ...
      writeSync(
        2,
        `electron-mcp: ELECTRON_MCP_RUNTIME=oam but ${oam} is older than oam ${min}.\n` +
    ...
    process.stderr.write(`electron-mcp: oam at ${oam} is older than ${min}; using Node instead.\n`);
    await runInProcess();
  } else {
    ...
    child.on("error", (err) => {
      if (spawned) return;
      if (mode === "oam") {
        process.stderr.write(`electron-mcp: failed to launch oam (${err.message})\n`);
        process.exit(1);
      }
      void runInProcess();
    });
