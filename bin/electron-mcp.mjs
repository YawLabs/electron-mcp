 ...
 * app's DevTools endpoint lands on. Every grant would be wide open, so
 * `--permission` is not wired up here.
 *
 * MINIMUM OAM VERSION
...
if (mode === "node") {
  ...
  if (!oam) {
    ...
    if (mode === "oam") {
      ...
      writeSync(
        2,
        "electron-mcp: ELECTRON_MCP_RUNTIME=oam but no runnable oam binary was found.\n" + shimNote +
          "Install from https://oamjs.org, set OAM_BIN=/path/to/oam, or use ELECTRON_MCP_RUNTIME=node.\n",
      );
      process.exit(1);
    }
    // auto: falling back is correct, but silence is how someone never learns
    // their oam install is a shape this launcher skips.
    if (oamShim) await errSync(`electron-mcp: ${shimNote}Using Node instead.\n`);
    await runInProcess();
    ...
    const remedy = found
      ? "Run \`oam self-update\`, or use ELECTRON_MCP_RUNTIME=node.\n"
      : "Check that it is an executable oam binary for this platform, or use ELECTRON_MCP_RUNTIME=node.\n";
    if (mode === "oam") {
      await errSync(`electron-mcp: ELECTRON_MCP_RUNTIME=oam but ${detail}.\n${remedy}`);
      process.exit(1);
    }
    ...
    await errSync(`electron-mcp: ${detail}; using Node instead.\n`);
    await runInProcess();
  } else {
    ...
    const launchFailed = async (err) => {
      if (mode === "oam") {
        await errSync(`electron-mcp: failed to launch oam (${err?.message ?? err})\n`);
        process.exit(1);
      }
      await runInProcess();
    };
