 ...
 *
 * Why it matters here specifically: .github holds only CODEOWNERS -- there is
 * no workflow of any kind, and GitHub Actions is disabled on this repo -- so
 * there is no runner to arbitrate formatting later. (release.sh has CI-mode
 * branches keyed on .github/workflows/release.yml, but no such workflow
 * exists.) Whatever this script reports is the ONLY lint signal that exists
 ...
 *
 * It used to be read out of biome.json's `$schema` URL, which is wrong even
 * where it happens to give the right answer. `$schema` pins the version the
 * CONFIG is validated against; it is not the binary the repo installs, and the
 * two drift apart the moment a `^`/`~` range resolves forward while the
 * hand-written schema URL stays where it was. They agree here today -- both
 * say 1.9.4 -- but the devDependency is `^1.9.4`, so any later 1.x release
 * would be picked up by `npm install` while $schema stayed put, and this host
 * would have gone on linting with a binary the repo no longer uses. Sibling
 * repos had already drifted that way. Reading the lockfile removes the
 * coupling instead of relying on the two staying in step.
 */
